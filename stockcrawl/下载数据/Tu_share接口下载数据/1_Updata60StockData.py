...
StockDatapath = 'd:\\StockFile\\StockData_60'
StockDatapath2 = 'd:\\StockFile\\StockData_60\\000002.csv'
...
def updateFile(FilePath,StockCode):
    ...
    if lastDay.replace('0', '') != today.replace('0', ''):
        try:
            StockData = ts.get_k_data(StockCode[0:6], start=lastDay, end=today,ktype='60')  # 取得数据
        except:
            print('Error')
        else:
