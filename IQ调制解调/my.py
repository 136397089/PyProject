...
t = [step*i for i in range(TimeSize / step)]
si = []
ci = []
...
for i in t:
    si.append(math.sin(i*2*math.pi*fhz))
    ci.append(math.cos(i*2*math.pi*fhz))
...
plt.plot(t, ci) # 绘制曲线 y1
plt.show()



def GetIQSin(Step,TIme)
    for i in Num:
        si.append(math.sin(i*Step))
        ci.append(math.cos(i*Step))
